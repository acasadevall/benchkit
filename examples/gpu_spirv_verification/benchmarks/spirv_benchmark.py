    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if curr_logger is None:
                ...
                _msg = func.__name__ if message is None else message
...
@add_log()
def get_platform_gpu_devices(self, platform: Platform) -> List[str]:
    if Platform is None:
        return
...
@clean_up_ssh_connection
@add_log()
def get_platform_existing_tests(self, platform: Platform) -> List[str]:
    if Platform is None:
        return
    list_bin_tests = platform.comm.shell(
        command=shlex.split(f"find ~/gpu-verification-master/examples/vulkan-cl-spirv-runtime/test/build/bin -type f | grep vk_spirv_runtime"),
        shell=True,
        print_input=False,
        print_output=False,
        output_is_log=False,            
...
@clean_up_ssh_connection
@add_log()
def prepare_platform_tests(self, platform: Platform, benchmark_name: str, re_generate_is_needed: bool = True, is_dry_run: bool = False, filter_tests: List[str] = None) -> bool:
    if Platform is None:
        return
    ...
    command = shlex.split(
        f"SKIP={skip_tests} ~/gpu-verification-master/examples/vulkan-cl-spirv-runtime/test/scripts/benchmark.launch -tn \"{benchmark_name}\" -g \
        {regex_tests} \
        {dryrun_flag}"
    )
    ...
    return status != "" # TODO: we need status ...
...
class SpirvBenchmark(Benchmark):
    ...
    def __init__(
        ...
        list_vk_devices = get_platform_gpu_devices(self, platform)
        ...
        if device_id < 0:
            self.curr_logger.error(f"{device_id} is not a valid deice id (device_id >= 0)")
            self.ill_formed = True
            return
        ...
        #TODO: add following stuff into PreHook
        existing_tests = get_platform_existing_tests(self, platform)
        valid_tests = []
        ...
        if re_generate_always:
            ...
            if proposed_bm_name in valid_tests:
                ...
                pass
        proposed_bm_name = proposed_bm_name
        prepare_platform_tests(self, platform, proposed_bm_name, re_generate_is_needed=need_generate_test, is_dry_run=is_dry_run, filter_tests=litmus_tests)
    ...
    def single_run(
        ...
        **kwargs,
        ...
        test_exec_bin_path = pathlib.Path("~/gpu-verification-master/examples/vulkan-cl-spirv-runtime/test/build/bin") / bin_wg_sync_exe
        ...
        split_command = shlex.split(
            ...
            {'-glmem' if runtime_test_variables.get('glmem') else ''} \
            {'-shmem' if runtime_test_variables.get('shmem') else ''} \
            {regex_tests}"
        )
